...
{
    public class DeclareHuaxingTalisman
    {
        #region //静态变量定义
        ...
        public delegate bool ForeachHandler(DeclareHuaxingTalisman cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareHuaxingTalisman> _dataCaches = null;
        public static Dictionary<int, DeclareHuaxingTalisman> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("HuaxingTalisman", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareHuaxingTalisman cfg = null;
        ...
        private unsafe static DeclareHuaxingTalisman LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareHuaxingTalisman();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("HuaxingTalisman", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareHuaxingTalisman>();
        ...
        //根据ID获取数据
        public static DeclareHuaxingTalisman Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareHuaxingTalisman result = null;
                var keyIndex = 0;
