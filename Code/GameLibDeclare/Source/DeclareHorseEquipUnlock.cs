//文件是自动生成,请勿手动修改.来自数据文件:Horse_equip_unlock
using System;
using System.Collections.Generic;
namespace GameLib.Cfg.Data
{
    public class DeclareHorseEquipUnlock
    {
        #region //静态变量定义
        ...
        public delegate bool ForeachHandler(DeclareHorseEquipUnlock cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareHorseEquipUnlock> _dataCaches = null;
        public static Dictionary<int, DeclareHorseEquipUnlock> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("HorseEquipUnlock", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareHorseEquipUnlock cfg = null;
        ...
        private unsafe static DeclareHorseEquipUnlock LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareHorseEquipUnlock();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("HorseEquipUnlock", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareHorseEquipUnlock>(keyCount);
        ...
        //根据ID获取数据
        public static DeclareHorseEquipUnlock Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareHorseEquipUnlock result = null;
                var keyIndex = 0;
