//文件是自动生成,请勿手动修改.来自数据文件:Cross_devil_card_Train
using System;
using System.Collections.Generic;
namespace GameLib.Cfg.Data
{
    public class DeclareCrossDevilCardTrain
    {
        #region //静态变量定义
        ...
        public delegate bool ForeachHandler(DeclareCrossDevilCardTrain cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareCrossDevilCardTrain> _dataCaches = null;
        public static Dictionary<int, DeclareCrossDevilCardTrain> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("CrossDevilCardTrain", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareCrossDevilCardTrain cfg = null;
        ...
        private unsafe static DeclareCrossDevilCardTrain LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareCrossDevilCardTrain();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("CrossDevilCardTrain", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareCrossDevilCardTrain>(keyCount);
        ...
        //根据ID获取数据
        public static DeclareCrossDevilCardTrain Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareCrossDevilCardTrain result = null;
                var keyIndex = 0;
