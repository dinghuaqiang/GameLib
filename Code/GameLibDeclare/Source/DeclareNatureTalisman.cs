//文件是自动生成,请勿手动修改.来自数据文件:NatureTalisman
using System;
using System.Collections.Generic;
namespace GameLib.Cfg.Data
{
    public class DeclareNatureTalisman
    {
        #region //静态变量定义
        ...
        public delegate bool ForeachHandler(DeclareNatureTalisman cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareNatureTalisman> _dataCaches = null;
        public static Dictionary<int, DeclareNatureTalisman> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("NatureTalisman", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareNatureTalisman cfg = null;
        ...
        private unsafe static DeclareNatureTalisman LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareNatureTalisman();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("NatureTalisman", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareNatureTalisman>();
        ...
        //根据ID获取数据
        public static DeclareNatureTalisman Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareNatureTalisman result = null;
                var keyIndex = 0;
