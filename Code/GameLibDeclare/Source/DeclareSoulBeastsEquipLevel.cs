...
namespace GameLib.Cfg.Data
{
    public class DeclareSoulBeastsEquipLevel
    {
        ...
        private static int _needExp_Index = -1;
        #endregion
        #region //私有变量定义
        //ID
        private int _id;
        ...
        public int NeedExp { get{ return _needExp; }}
        #endregion

        #region //静态变量以及方法定义
        //线程锁对象
        private static object _lockObj = new object();
        public delegate bool ForeachHandler(DeclareSoulBeastsEquipLevel cfg);
        //数据存储字典
            ...
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("SoulBeastsEquipLevel", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareSoulBeastsEquipLevel cfg = null;
        ...
        private unsafe static DeclareSoulBeastsEquipLevel LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareSoulBeastsEquipLevel();
            ...
            tmp._needExp = (int)GetValue(keyIndex, _needExp_Index, ptr);
            return tmp;
        }
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("SoulBeastsEquipLevel", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    if (!nameIndexs.TryGetValue("NeedExp", out _needExp_Index)) _needExp_Index = -1;
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareSoulBeastsEquipLevel>(keyCount);
        ...
        public static DeclareSoulBeastsEquipLevel Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareSoulBeastsEquipLevel result = null;
                var keyIndex = 0;
