...
{
    public class DeclareGuildBattleScore
    {
        #region //静态变量定义
        ...
        public delegate bool ForeachHandler(DeclareGuildBattleScore cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareGuildBattleScore> _dataCaches = null;
        public static Dictionary<int, DeclareGuildBattleScore> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("GuildBattleScore", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareGuildBattleScore cfg = null;
        ...
        private unsafe static DeclareGuildBattleScore LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareGuildBattleScore();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("GuildBattleScore", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareGuildBattleScore>(keyCount);
        ...
        //根据ID获取数据
        public static DeclareGuildBattleScore Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareGuildBattleScore result = null;
                var keyIndex = 0;
