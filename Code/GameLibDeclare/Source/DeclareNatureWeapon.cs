...
{
    public class DeclareNatureWeapon
    {
        #region //静态变量定义
        ...
        public delegate bool ForeachHandler(DeclareNatureWeapon cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareNatureWeapon> _dataCaches = null;
        public static Dictionary<int, DeclareNatureWeapon> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("NatureWeapon", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareNatureWeapon cfg = null;
        ...
        private unsafe static DeclareNatureWeapon LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareNatureWeapon();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("NatureWeapon", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareNatureWeapon>(keyCount);
        ...
        //根据ID获取数据
        public static DeclareNatureWeapon Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareNatureWeapon result = null;
                var keyIndex = 0;
