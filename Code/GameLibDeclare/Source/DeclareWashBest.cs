//文件是自动生成,请勿手动修改.来自数据文件:wash_best
using System;
using System.Collections.Generic;
namespace GameLib.Cfg.Data
{
    public class DeclareWashBest
    {
        #region //静态变量定义
        ...
        public delegate bool ForeachHandler(DeclareWashBest cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareWashBest> _dataCaches = null;
        public static Dictionary<int, DeclareWashBest> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("WashBest", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareWashBest cfg = null;
        ...
        private unsafe static DeclareWashBest LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareWashBest();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("WashBest", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareWashBest>(keyCount);
        ...
        //根据ID获取数据
        public static DeclareWashBest Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareWashBest result = null;
                var keyIndex = 0;
