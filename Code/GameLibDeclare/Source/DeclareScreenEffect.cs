...
namespace GameLib.Cfg.Data
{
    public class DeclareScreenEffect
    {
        ...
        public delegate bool ForeachHandler(DeclareScreenEffect cfg);
        //数据存储字典
            ...
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("ScreenEffect", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareScreenEffect cfg = null;
        ...
        private unsafe static DeclareScreenEffect LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareScreenEffect();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("ScreenEffect", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareScreenEffect>(keyCount);
        ...
        public static DeclareScreenEffect Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareScreenEffect result = null;
                var keyIndex = 0;
