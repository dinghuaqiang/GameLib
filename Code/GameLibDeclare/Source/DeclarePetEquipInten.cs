//文件是自动生成,请勿手动修改.来自数据文件:pet_equip_inten
using System;
using System.Collections.Generic;
namespace GameLib.Cfg.Data
{
    public class DeclarePetEquipInten
    {
        #region //静态变量定义
        ...
        public delegate bool ForeachHandler(DeclarePetEquipInten cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclarePetEquipInten> _dataCaches = null;
        public static Dictionary<int, DeclarePetEquipInten> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("PetEquipInten", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclarePetEquipInten cfg = null;
        ...
        private unsafe static DeclarePetEquipInten LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclarePetEquipInten();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("PetEquipInten", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclarePetEquipInten>(keyCount);
        ...
        //根据ID获取数据
        public static DeclarePetEquipInten Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclarePetEquipInten result = null;
                var keyIndex = 0;
