//文件是自动生成,请勿手动修改.来自数据文件:pet_equip_synthesis
using System;
using System.Collections.Generic;
namespace GameLib.Cfg.Data
{
    public class DeclarePetEquipSynthesis
    {
        #region //静态变量定义
        ...
        public delegate bool ForeachHandler(DeclarePetEquipSynthesis cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclarePetEquipSynthesis> _dataCaches = null;
        public static Dictionary<int, DeclarePetEquipSynthesis> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("PetEquipSynthesis", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclarePetEquipSynthesis cfg = null;
        ...
        private unsafe static DeclarePetEquipSynthesis LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclarePetEquipSynthesis();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("PetEquipSynthesis", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclarePetEquipSynthesis>(keyCount);
        ...
        //根据ID获取数据
        public static DeclarePetEquipSynthesis Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclarePetEquipSynthesis result = null;
                var keyIndex = 0;
