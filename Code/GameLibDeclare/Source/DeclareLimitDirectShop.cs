//文件是自动生成,请勿手动修改.来自数据文件:limit_direct_shop
using System;
using System.Collections.Generic;
namespace GameLib.Cfg.Data
{
    public class DeclareLimitDirectShop
    {
        #region //静态变量定义
        ...
        //折扣（万分比，程序取值的时候会除以一万显示）
        //配空等于不显示折扣
        //（hide）
        private int _discount;
        //原价，单位（分），
        //折后价格读取的rechargeItem表格
        //（hide）
        ...
        public delegate bool ForeachHandler(DeclareLimitDirectShop cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareLimitDirectShop> _dataCaches = null;
        public static Dictionary<int, DeclareLimitDirectShop> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("LimitDirectShop", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareLimitDirectShop cfg = null;
        ...
        private unsafe static DeclareLimitDirectShop LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareLimitDirectShop();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("LimitDirectShop", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareLimitDirectShop>(keyCount);
        ...
        //根据ID获取数据
        public static DeclareLimitDirectShop Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareLimitDirectShop result = null;
                var keyIndex = 0;
