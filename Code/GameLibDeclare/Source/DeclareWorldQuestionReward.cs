...
namespace GameLib.Cfg.Data
{
    public class DeclareWorldQuestionReward
    {
        ...
        public delegate bool ForeachHandler(DeclareWorldQuestionReward cfg);
        //数据存储字典
            ...
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("WorldQuestionReward", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareWorldQuestionReward cfg = null;
        ...
        private unsafe static DeclareWorldQuestionReward LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareWorldQuestionReward();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("WorldQuestionReward", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareWorldQuestionReward>(keyCount);
        ...
        public static DeclareWorldQuestionReward Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareWorldQuestionReward result = null;
                var keyIndex = 0;
