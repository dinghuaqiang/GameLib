//文件是自动生成,请勿手动修改.来自数据文件:Treasure_xianjiaSecret
using System;
using System.Collections.Generic;
namespace GameLib.Cfg.Data
{
    public class DeclareTreasureXianjiaSecret
    {
        #region //静态变量定义
        ...
        public delegate bool ForeachHandler(DeclareTreasureXianjiaSecret cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareTreasureXianjiaSecret> _dataCaches = null;
        public static Dictionary<int, DeclareTreasureXianjiaSecret> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("TreasureXianjiaSecret", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareTreasureXianjiaSecret cfg = null;
        ...
        private unsafe static DeclareTreasureXianjiaSecret LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareTreasureXianjiaSecret();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("TreasureXianjiaSecret", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareTreasureXianjiaSecret>(keyCount);
        ...
        //根据ID获取数据
        public static DeclareTreasureXianjiaSecret Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareTreasureXianjiaSecret result = null;
                var keyIndex = 0;
