//文件是自动生成,请勿手动修改.来自数据文件:state_stifle_add_level
using System;
using System.Collections.Generic;
namespace GameLib.Cfg.Data
{
    public class DeclareStateStifleAddLevel
    {
        #region //静态变量定义
        ...
        public delegate bool ForeachHandler(DeclareStateStifleAddLevel cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareStateStifleAddLevel> _dataCaches = null;
        public static Dictionary<int, DeclareStateStifleAddLevel> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("StateStifleAddLevel", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareStateStifleAddLevel cfg = null;
        ...
        private unsafe static DeclareStateStifleAddLevel LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareStateStifleAddLevel();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("StateStifleAddLevel", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareStateStifleAddLevel>(keyCount);
        ...
        //根据ID获取数据
        public static DeclareStateStifleAddLevel Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareStateStifleAddLevel result = null;
                var keyIndex = 0;
