//文件是自动生成,请勿手动修改.来自数据文件:clone_xinmo
using System;
using System.Collections.Generic;
namespace GameLib.Cfg.Data
{
    public class DeclareCloneXinmo
    {
        #region //静态变量定义
        ...
        public delegate bool ForeachHandler(DeclareCloneXinmo cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareCloneXinmo> _dataCaches = null;
        public static Dictionary<int, DeclareCloneXinmo> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("CloneXinmo", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareCloneXinmo cfg = null;
        ...
        private unsafe static DeclareCloneXinmo LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareCloneXinmo();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("CloneXinmo", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareCloneXinmo>(keyCount);
        ...
        //根据ID获取数据
        public static DeclareCloneXinmo Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareCloneXinmo result = null;
                var keyIndex = 0;
