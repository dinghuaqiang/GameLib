//文件是自动生成,请勿手动修改.来自数据文件:FallingSkyTwo_Level
using System;
using System.Collections.Generic;
namespace GameLib.Cfg.Data
{
    public class DeclareFallingSkyTwoLevel
    {
        #region //静态变量定义
        ...
        public delegate bool ForeachHandler(DeclareFallingSkyTwoLevel cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareFallingSkyTwoLevel> _dataCaches = null;
        public static Dictionary<int, DeclareFallingSkyTwoLevel> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("FallingSkyTwoLevel", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareFallingSkyTwoLevel cfg = null;
        ...
        private unsafe static DeclareFallingSkyTwoLevel LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareFallingSkyTwoLevel();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("FallingSkyTwoLevel", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareFallingSkyTwoLevel>(keyCount);
        ...
        //根据ID获取数据
        public static DeclareFallingSkyTwoLevel Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareFallingSkyTwoLevel result = null;
                var keyIndex = 0;
