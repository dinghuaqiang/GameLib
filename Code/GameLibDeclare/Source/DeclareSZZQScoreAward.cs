...
namespace GameLib.Cfg.Data
{
    public class DeclareSZZQScoreAward
    {
        ...
        public delegate bool ForeachHandler(DeclareSZZQScoreAward cfg);
        //数据存储字典
            ...
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("SZZQScoreAward", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareSZZQScoreAward cfg = null;
        ...
        private unsafe static DeclareSZZQScoreAward LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareSZZQScoreAward();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("SZZQScoreAward", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareSZZQScoreAward>(keyCount);
        ...
        public static DeclareSZZQScoreAward Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareSZZQScoreAward result = null;
                var keyIndex = 0;
