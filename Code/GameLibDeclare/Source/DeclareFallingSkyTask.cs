...
{
    public class DeclareFallingSkyTask
    {
        #region //静态变量定义
        ...
        public delegate bool ForeachHandler(DeclareFallingSkyTask cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareFallingSkyTask> _dataCaches = null;
        public static Dictionary<int, DeclareFallingSkyTask> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("FallingSkyTask", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareFallingSkyTask cfg = null;
        ...
        private unsafe static DeclareFallingSkyTask LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareFallingSkyTask();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("FallingSkyTask", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareFallingSkyTask>(keyCount);
        ...
        //根据ID获取数据
        public static DeclareFallingSkyTask Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareFallingSkyTask result = null;
                var keyIndex = 0;
