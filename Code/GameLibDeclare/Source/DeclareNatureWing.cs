//文件是自动生成,请勿手动修改.来自数据文件:NatureWing
using System;
using System.Collections.Generic;
namespace GameLib.Cfg.Data
{
    public class DeclareNatureWing
    {
        #region //静态变量定义
        ...
        public delegate bool ForeachHandler(DeclareNatureWing cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareNatureWing> _dataCaches = null;
        public static Dictionary<int, DeclareNatureWing> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("NatureWing", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareNatureWing cfg = null;
        ...
        private unsafe static DeclareNatureWing LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareNatureWing();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("NatureWing", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareNatureWing>(keyCount);
        ...
        //根据ID获取数据
        public static DeclareNatureWing Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareNatureWing result = null;
                var keyIndex = 0;
