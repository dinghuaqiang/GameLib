//文件是自动生成,请勿手动修改.来自数据文件:bossnew_SoulBeasts
using System;
using System.Collections.Generic;
namespace GameLib.Cfg.Data
{
    public class DeclareBossnewSoulBeasts
    {
        #region //静态变量定义
        ...
        //类型1.兽灵/兽神水晶；2.兽血水晶；3.精英怪；4.Boss
        private int _type;
        //怪物头像 hide
        ...
        public delegate bool ForeachHandler(DeclareBossnewSoulBeasts cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareBossnewSoulBeasts> _dataCaches = null;
        public static Dictionary<int, DeclareBossnewSoulBeasts> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("BossnewSoulBeasts", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareBossnewSoulBeasts cfg = null;
        ...
        private unsafe static DeclareBossnewSoulBeasts LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareBossnewSoulBeasts();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("BossnewSoulBeasts", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareBossnewSoulBeasts>(keyCount);
        ...
        //根据ID获取数据
        public static DeclareBossnewSoulBeasts Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareBossnewSoulBeasts result = null;
                var keyIndex = 0;
