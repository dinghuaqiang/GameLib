//文件是自动生成,请勿手动修改.来自数据文件:Cross_devil_Group_Boss
using System;
using System.Collections.Generic;
namespace GameLib.Cfg.Data
{
    public class DeclareCrossDevilGroupBoss
    {
        #region //静态变量定义
        ...
        public int RefreshTime { get{ return _refresh_time; }}
        ...
        public delegate bool ForeachHandler(DeclareCrossDevilGroupBoss cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareCrossDevilGroupBoss> _dataCaches = null;
        public static Dictionary<int, DeclareCrossDevilGroupBoss> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("CrossDevilGroupBoss", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareCrossDevilGroupBoss cfg = null;
        ...
        private unsafe static DeclareCrossDevilGroupBoss LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareCrossDevilGroupBoss();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("CrossDevilGroupBoss", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareCrossDevilGroupBoss>(keyCount);
        ...
        //根据ID获取数据
        public static DeclareCrossDevilGroupBoss Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareCrossDevilGroupBoss result = null;
                var keyIndex = 0;
