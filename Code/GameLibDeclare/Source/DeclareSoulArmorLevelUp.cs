//文件是自动生成,请勿手动修改.来自数据文件:SoulArmor_level_up
using System;
using System.Collections.Generic;
namespace GameLib.Cfg.Data
{
    public class DeclareSoulArmorLevelUp
    {
        #region //静态变量定义
        ...
        public delegate bool ForeachHandler(DeclareSoulArmorLevelUp cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclareSoulArmorLevelUp> _dataCaches = null;
        public static Dictionary<int, DeclareSoulArmorLevelUp> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("SoulArmorLevelUp", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclareSoulArmorLevelUp cfg = null;
        ...
        private unsafe static DeclareSoulArmorLevelUp LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclareSoulArmorLevelUp();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("SoulArmorLevelUp", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclareSoulArmorLevelUp>(keyCount);
        ...
        //根据ID获取数据
        public static DeclareSoulArmorLevelUp Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclareSoulArmorLevelUp result = null;
                var keyIndex = 0;
