//文件是自动生成,请勿手动修改.来自数据文件:pet_equip_soulbound
using System;
using System.Collections.Generic;
namespace GameLib.Cfg.Data
{
    public class DeclarePetEquipSoulbound
    {
        #region //静态变量定义
        ...
        public int PercentValue { get{ return _percentValue; }}
        ...
        public delegate bool ForeachHandler(DeclarePetEquipSoulbound cfg);
        //数据存储字典
        private static bool _dataCacheFull = false;
        private static Dictionary<int, DeclarePetEquipSoulbound> _dataCaches = null;
        public static Dictionary<int, DeclarePetEquipSoulbound> CacheData
        {
            get 
            {
                lock (_lockObj)
                {
                    ...
                    if(!_dataCacheFull)
                    {
                        _dataCacheFull = true;
                        //读取所有数据
                        if (HanderDefine.DataCallBack("PetEquipSoulbound", out _compressData))
                        {
                            var iter = _dataIndexCaches.GetEnumerator();
                            while (iter.MoveNext())
                            {
                                DeclarePetEquipSoulbound cfg = null;
        ...
        private unsafe static DeclarePetEquipSoulbound LoadSingleData(int keyIndex)
        {
            int* ptr = (int*)_compressData.ToPointer();
            var tmp = new DeclarePetEquipSoulbound();
        ...
        private static void SetBaseData()
        {
            if (_dataCaches == null)
            {
                ...
                var _isExist = HanderDefine.BaseDataCallBack("PetEquipSoulbound", out keyCount, out _indexFactor, out _compressData, out columnShiftAndList, out _compressMaxColumn, out nameIndexs);
                if (_isExist)
                {
                    ...
                    unsafe
                    {
                        ...
                        _dataCaches = new Dictionary<int, DeclarePetEquipSoulbound>(keyCount);
        ...
        //根据ID获取数据
        public static DeclarePetEquipSoulbound Get(int id)
        {
            lock (_lockObj)
            {
                if (_dataCaches == null)
                    SetBaseData();
                DeclarePetEquipSoulbound result = null;
                var keyIndex = 0;
